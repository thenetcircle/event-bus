...
import com.thenetcircle.event_bus.story.tasks.http.FirebaseSinkBuilder

import scala.concurrent.duration._

class FirebaseSinkBuilderTest extends TestBase {

  behavior of "FirebaseSinkBuilder"

  val builder = new FirebaseSinkBuilder

  it should "build correct FirebaseSink with minimum config" in {
    ...
    settings.defaultRequest.uri shouldEqual Uri("https://fcm.googleapis.com/fcm/send")
    ...
    settings.connectionPoolSettings.get.maxOpenRequests shouldEqual 32
  ...
  it should "build correct sink with the full config" in {
    ...
    settings.defaultRequest.headers.size shouldEqual 4
